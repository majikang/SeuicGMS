    ...
    public function upload()
    {
        ...
        try {
            ...
            if( ! Storage::put($savePath, file_get_contents($this->file->getRealPath()))){
                return ['error' => '上传文件移动保存失败', 'status' => 0];
            }

            if( ! Storage::exists($savePath)){
                return ['error' => '上传文件不存在', 'status' => 0];
            }
            ...
            return ['success' => '上传文件成功', 'status' => 1, 'data' => $info];
        }
        catch (\Exception $e) {
            return ['error' => $e->getMessage() . 'Catch Error', 'status' => 0];
        }
    }
}